...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if not tickers_lst:
            raise Exception("The tickers list provided is empty.")
        for ticker in tickers_lst:
            if ticker not in tic_exchange_dic:
                raise Exception(f"The ticker '{ticker}' is not a key in the provided tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if not col_lst:
            raise Exception("The columns list provided is empty.")
        for col in col_lst:
            if col not in COLUMNS:
                raise Exception(f"The column '{col}' is not found in the provided COLUMNS list.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        raw_data = read_dat(tic)
        converted_data = [line_to_dict(line) for line in raw_data]
        filtered_data = [{col: dat[col] for col in col_lst} for dat in converted_data]
        data_dict[tic] = {
